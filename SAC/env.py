...
class RocketLander(gym.Env):
    ...
    def step(self, action):
        ...
        #     # shaping = -1 * (distance + speed + abs(angle) ** 2)
        #     shaping = -1 * (distance + speed + np.exp(abs(angle)) - 1.0)

        #     shaping += 0.1 * (self.legs[0].ground_contact + self.legs[1].ground_contact)

        #     # Additional reward for staying straight (low angular deviation)
...
if __name__ == "__main__":
    ...
    for _ in range(1000):
        # Take a random action (replace this with your own action logic)
